window.onload = () => {
  ...
  const salvaLocalStorage = () => {
    ...
    for (let index = 0; index < allPixels.length; index += 1) {
      listPixel.push(allPixels[index].style.backgroundColor);
    // const cor = allPixels[index].style.backgroundColor;
    }
    localStorage.setItem('pixelBoard', JSON.stringify(listPixel));
  ...
  const recuperaPixelBoard = () => {
    if (localStorage.getItem('pixelBoard')) {
      const status = JSON.parse(localStorage.getItem('pixelBoard'));
      const allPixels = document.querySelectorAll('.pixel');
      for (let index = 0; index < allPixels.length; index += 1) {
        allPixels[index].style.backgroundColor = status[index];
      }
    }
  };
  recuperaPixelBoard();
...
//   const pixelList = [];
//   for(let index = 0; index < allPixels.length; index += 1) {
//    pixelList.push(allPixels[index].style.backgroundColor = corSelected);
//    console.log(allPixels[index].style.backgroundColor);
//   }
//   localStorage.setItem('pixelBoard', JSON.stringify(pixelList))
// };
// savePixelBoard();
// const recuperaPixelBoard = () => {
//   if(localStorage.getItem('pixelBoard')) {
...
//     const allPixels = document.querySelectorAll('.pixel');
//     for(let index = 0; index < allPixels.length; index += 1) {
//       allPixels[index].style.backgroundColor = status;
//     }
//   }
// }
// recuperaPixelBoard();
